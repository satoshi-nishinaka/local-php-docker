...
class EvalExecutor
{
    ...
    public function output(): void
    {
        ...
        if ($this->convert !== 'eval') {
            ...
            switch ($this->convert) {
                case 'strtolowwer':
                    $buffer[] = '<textarea rows="20" cols="" style="width:90%;height:40%;" >' . strtolower($this->receiveString) . '</textarea><br />';
                    break;
                case  'strtoupper':
                    $buffer[] = '<textarea rows="20" cols="" style="width:90%;height:40%;" >' . strtoupper($this->receiveString) . '</textarea><br />';
                    break;
                case 'mb_convert_hankaku':
                    // 入力文字列を半角化の場合
                    $buffer[] = '<textarea rows="20" cols="" style="width:90%;height:40%;" >' . mb_convert_kana($this->receiveString, "as") . '</textarea><br />';
                    break;
                case 'print':
    ...
    private function initialize(): void
    {
        ...
        // POSTされた文字列を格納
        $this->receiveString = filter_input(INPUT_POST, 'exescript');
    ...
    /**
     * 入力されたテキストをPHPのコードと解釈し、コードを実行する
     *
     * @return void
     */
    ...
    {
        $buffer = [];
        $script = preg_replace("/(^<\?(php)?)|(\?>$)/", "", trim($this->receiveString));
    ...
    private function _execute($mode): void
    {
        switch ($mode) {
            ...
            case 'indent' :
                // インデント付与の場合
                if (strlen($this->receiveString) == 0) {
                    return;
                }
                $outputText = '';
                ...
                $script = trim(stripslashes($this->receiveString));
                ...
                return;
            case 'strlen' :
                echo "<input type='textbox' style='width:90%;' onFocus='this.select();' value='" . strlen($this->receiveString) . "' />\n";
                if (str_contains($this->receiveString, "\r\n")) {
                    echo "<p>&yen;r&yen;n込み</p>";
                } else if (strpos($this->receiveString, "\n") !== false) {
                    echo "<p>&yen;n込み</p>";
                }
                return;
            case "bin2hex" :
                echo '<textarea rows="20" class="form-control" onFocus="this.select();">' . bin2hex($this->receiveString) . "</textarea>\n";
                return;
            case "baseconvert16" :
                echo '<textarea rows="20" class="form-control" onFocus="this.select();">' . base_convert($this->receiveString, 10, 16) . "</textarea>\n";
                return;
            case "baseconvert10" :
                echo '<textarea rows="20" class="form-control" onFocus="this.select();">' . base_convert($this->receiveString, 16, 10) . "</textarea>\n";
                return;
        }
    }
